import { SalaryPeriod, PeriodAfore, AforeResult } from "./types";
...
/**
 * Calcula el saldo AFORE para cada periodo salarial.
 *
 * Cada SalaryPeriod tiene fechaInicio, fechaFin, y salarioDiario.
 * Las contribuciones se calculan como: dias * salarioDiario * tasa.
 *
 * Reglas por año (del inicio del periodo):
 ...
 *
 * Rendimientos:
 * - SAR/RCV: IF(year > 1996) → (2026 - year) * 0.1048 * aportación
 * - Vivienda: siempre → (2026 - year) * 0.1048 * aportación
 */
export function calculateAfore(salaryPeriods: SalaryPeriod[]): AforeResult {
  const periods: PeriodAfore[] = salaryPeriods.map((sp) => {
    ...
    const rcvTrabajadorRendimientos = rendCondition ? rendimientoFactor * rcvTrabajador : 0;
    const rcvPatronRendimientos = rendCondition ? rendimientoFactor * rcvPatron : 0;
  ...
  const totals = periods.reduce(
    (acc, p) => ({
      ...
      rcvTrabajadorRendimientos: acc.rcvTrabajadorRendimientos + p.rcvTrabajadorRendimientos,
      rcvPatron: acc.rcvPatron + p.rcvPatron,
      rcvPatronRendimientos: acc.rcvPatronRendimientos + p.rcvPatronRendimientos,
      vivienda92: acc.vivienda92 + p.vivienda92,
      vivienda92Rendimientos: acc.vivienda92Rendimientos + p.vivienda92Rendimientos,
      vivienda97: acc.vivienda97 + p.vivienda97,
      vivienda97Rendimientos: acc.vivienda97Rendimientos + p.vivienda97Rendimientos,
    }),
    {
      sar92: 0, sar92Rendimientos: 0,
      sar97: 0, sar97Rendimientos: 0,
      rcvTrabajador: 0, rcvTrabajadorRendimientos: 0,
      rcvPatron: 0, rcvPatronRendimientos: 0,
      vivienda92: 0, vivienda92Rendimientos: 0,
      vivienda97: 0, vivienda97Rendimientos: 0,
    }
  );

  const saldoAfore =
    totals.sar92 + totals.sar92Rendimientos +
    totals.sar97 + totals.sar97Rendimientos +
    totals.vivienda92 + totals.vivienda92Rendimientos +
    totals.vivienda97 + totals.vivienda97Rendimientos;

  const saldoRCV =
    totals.rcvTrabajador + totals.rcvTrabajadorRendimientos +
    totals.rcvPatron + totals.rcvPatronRendimientos;
